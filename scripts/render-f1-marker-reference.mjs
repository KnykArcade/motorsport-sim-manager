...
function carLayers(key, design, primary, accent) {
  const wheels = design.wheels.map((w) =>
    `<rect x="${w.x}" y="${w.y}" width="${w.width}" height="${w.height}" rx="${w.rx}" fill="#08090b" stroke="#41464d" stroke-width=".35"/>`,
  ).join('');
  const suspension = design.detailPaths.map((d, index) =>
    `<path d="${d}" fill="none" stroke="${index === 0 ? '#111519' : '#d7d9dc'}" stroke-width="${index === 0 ? '.5' : '.24'}" stroke-linecap="round" opacity="${index === 0 ? '1' : '.75'}"/>`,
  ).join('');
  const accents = design.accentPaths.map((d, index) =>
    `<path d="${d}" fill="${index < 3 ? accent : 'none'}" stroke="${index < 3 ? '#07090b' : accent}" stroke-width="${index < 3 ? '.28' : '.38'}" stroke-linecap="round" stroke-linejoin="round"/>`,
  ).join('');
  const halo = design.haloPath
    ? `<path d="${design.haloPath}" fill="none" stroke="#111417" stroke-width=".48" stroke-linecap="round" stroke-linejoin="round"/>`
    : '';

  return `<g data-design="${key}">
    <path d="${design.outerPath}" fill="${primary}" stroke="#07090b" stroke-width="1.55" stroke-linejoin="round"/>
    ${wheels}${suspension}
    <path d="${design.rearWingPath}" fill="${accent}" stroke="#07090b" stroke-width=".42"/>
    <path d="${design.bodyPath}" fill="${primary}" stroke="#07090b" stroke-width=".45"/>
    <path d="${design.sidepodPath}" fill="${primary}" stroke="#07090b" stroke-width=".36"/>
    ${accents}
    <path d="${design.nosePath}" fill="${primary}" stroke="#07090b" stroke-width=".34"/>
    <ellipse cx="${design.cockpit.cx}" cy="${design.cockpit.cy}" rx="${design.cockpit.rx}" ry="${design.cockpit.ry}" fill="#080a0d" stroke="${accent}" stroke-width=".48"/>
    ...
    ${halo}
    <path d="${design.frontWingPath}" fill="${accent}" stroke="#07090b" stroke-width=".42"/>
    ...
    <path d="${design.outerPath}" fill="none" stroke="#f7f7f7" stroke-width=".58" stroke-linejoin="round"/>
  </g>`;
}
...
function card([key, primary, accent, number, features], index) {
  ...
  return `<g>
    ...
    <text x="${x + 364}" y="${y + 112}" fill="#f7f7f7" font-family="Arial Narrow,Arial,sans-serif" font-size="14" font-weight="900">35 PX · FRONT-WING NUMBER</text>
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="980" viewBox="0 0 1600 980">
  ...
  <text x="38" y="88" fill="#d0d3d7" font-family="Arial Narrow,Arial,sans-serif" font-size="17" font-weight="800">1990s / 2000s / 2010s / 2020s · 35 PX · TWO-COLOR RUNTIME LIVERY · FRONT-WING NUMBER</text>
  ${eras.map(card).join('')}
