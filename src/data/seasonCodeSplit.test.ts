...
describe('season code-splitting', () => {
  describe('season catalog (lightweight, no heavy data)', () => {
    it('exposes all 123 seasons (37 F1 + 31 IndyCar + 14 CART + 4 Champ Car + 37 NASCAR)', () => {
      ...
      expect(nascar.length).toBe(37);
      expect(availableSeasons.length).toBe(123);
    });
  ...
  describe('season loader (async, code-split)', () => {
    it('has loaders for all 123 seasons', () => {
      expect(getLoaderKeys().length).toBe(123);
    });
