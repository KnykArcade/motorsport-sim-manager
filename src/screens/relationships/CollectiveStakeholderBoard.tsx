      ...
      {profiles.length === 0 ? (
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          {profiles.map((profile) => (
            <article key={profile.id} className={`rounded-lg border p-3 ${STATUS_STYLES[profile.status]}`}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-sm font-bold text-neutral-100">{profile.title}</div>
                  <div className="mt-0.5 text-[10px] uppercase tracking-wide text-neutral-500">Authority #{profile.authorityRank} · Collective relationship</div>
                </div>
