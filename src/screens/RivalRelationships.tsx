...
export function RivalRelationships() {
  ...
  return <WorkspaceScreen>
    ...
    {tab === 'dossier' && selected && selectedRivalId && <Panel title="Rival Dossier">
      <div className="mb-4 flex flex-wrap gap-1">{relationships.map((item) => { const rivalId = rivalIdOf(item); return <button key={rivalId} type="button" onClick={() => setSelectedId(rivalId)} className={`rounded px-2 py-1 text-[10px] ${rivalId === selectedRivalId ? 'bg-sky-500/20 text-sky-200' : 'bg-neutral-900 text-neutral-500 hover:text-neutral-200'}`}>{teamName(rivalId)}</button>; })}</div>
      <div className="grid gap-4 lg:grid-cols-2"><div className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-4"><div className="flex items-start justify-between"><div><h2 className="text-lg font-bold text-neutral-100">{teamName(selectedRivalId)}</h2><p className="text-xs text-neutral-500">{rivalRelationshipLabel(selected.score)}</p></div><Score value={selected.score} /></div><div className="mt-4 grid grid-cols-2 gap-3"><Meter label="Sporting respect" value={selected.sportingRespect} /><Meter label="Political alignment" value={(selected.politicalAlignment + 100) / 2} display={selected.politicalAlignment} /><Meter label="Commercial trust" value={selected.commercialTrust} /><Meter label="Technical suspicion" value={selected.technicalSuspicion} danger /></div><div className="mt-4 flex flex-wrap gap-1">{selected.tags.map((tag) => <span key={tag} className="rounded bg-violet-500/10 px-2 py-1 text-[10px] text-violet-300">{splitLabel(tag)}</span>)}</div></div>
      <div className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-4"><h3 className="font-semibold text-neutral-100">Management Actions</h3><p className="mt-1 text-xs text-neutral-500">Actions change the relationship immediately and can affect politics, technical scrutiny, and market competition.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{(['OpenDialogue', 'TechnicalExchange', 'ScoutPersonnel', 'FileProtest'] as RivalAction[]).map((action) => <button key={action} type="button" disabled={budget < RIVAL_ACTION_COST[action]} onClick={() => dispatch({ type: 'TAKE_RIVAL_ACTION', rivalTeamId: selectedRivalId, action })} className="rounded border border-neutral-700 bg-neutral-950/50 p-3 text-left enabled:hover:border-amber-500/50 disabled:opacity-40"><div className="text-xs font-semibold text-amber-300">{splitLabel(action)}</div><p className="mt-1 text-[10px] text-neutral-500">{actionDescription(action)}</p><div className="mt-2 text-[10px] text-neutral-400">{RIVAL_ACTION_COST[action] ? formatMoney(RIVAL_ACTION_COST[action]) : 'No cost'}</div></button>)}</div></div></div>
    </Panel>}
