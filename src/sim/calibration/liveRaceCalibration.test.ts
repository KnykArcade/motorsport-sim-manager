...
describe('live race calibration aggregation', () => {
  it('averages race metrics and detects pit-call reconciliation failures', () => {
    const base = {
      carsStarted: 20, raceLaps: 60, playerCars: 2,
      carsWithThreePlusStops: 0, recommendationAppearances: 4, modeChanges: 20,
      safetyCarDeployments: 1, retirements: 2, totalEvents: 40, pitEventEntries: 10,
    ...
    expect(report.rates.eventsPerLap).toBeCloseTo(0.667, 3);
    expect(report.rates.pitEventEntriesPerStop).toBeCloseTo(20 / 38, 3);
