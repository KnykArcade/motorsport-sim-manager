...
export function progressAcademyMember(a: AcademyMember, boost = 0, planEffect = 0): AcademyMember {
  const gap = Math.max(0, a.potential - a.overall);
  const step = Math.min(gap, a.developmentRate * 0.15 * (1 + boost + planEffect));
