...
export const RACE_WEEKEND_PACKAGES: Record<RaceWeekendPackageType, PackageDefinition> = {
  ...
  MandatoryMinimum: {
    ...
    description: 'Emergency operations package for teams that cannot afford standard race operations. Gets the cars to the grid with skeleton staff, no practice, and a locked baseline setup. Devastating penalties across every dimension of the race weekend.',
    costModifier: 0.0,
    effects: {
      paceModifier: -1.0,
      reliabilityPrep: -0.7,
      ...
      sponsorSatisfaction: -40,
      driverMorale: -30,
      tyrePreservation: -0.35,
      developmentDataGain: 0.05,
      operationalRiskMultiplier: 2.5,
      crashRiskMultiplier: 1.6,
    },
    warnings: [
      'Devastating pace, reliability, and pit-stop penalties — cars will be off the back of the grid.',
      'No practice sessions or car setup changes — baseline setup only.',
      'Severe sponsor confidence loss and driver morale crash.',
      'Greatly elevated crash and operational risk across the weekend.',
      'Repeated use escalates financial distress and owner pressure rapidly.',
...
export function updateFinancialDistress(
  ...
  const ownerPressure = Math.min(100, Math.max(0,
    prev.ownerPressure + (usedEmergencyPackage ? 15 : 0) + (isNegative ? 5 : 0) - (level === 'Stable' ? 5 : 0),
  ));
  return {
    level,
